...
class Database:
    ...
    async def connect(self):
        ...
        try:
            ...
            logger.info("ᴄᴏɴɴᴇᴄᴛᴇᴅ ᴛᴏ ᴍᴏɴɢᴏᴅʙ sᴜᴄᴄᴇssꜰᴜʟʟʏ!")
            return True
    ...
    async def create_indexes(self):
        ...
        try:
            ...
            await self.operations.create_index([("user_id", ASCENDING), ("date", DESCENDING)])
            await self.operations.create_index("date", expireAfterSeconds=86400*30)  # 30 days
    ...
    async def redeem_premium_code(self, code: str, user_id: int):
        ...
        try:
            ...
            if not premium_code:
                logger.info(f"Premium code '{code}' not found or already used")
                return False
            
            logger.info(f"Found valid premium code '{code}' for {premium_code['days']} days")
            
            # Get user to check if they exist
            user = await self.get_user(user_id)
            if not user:
                logger.error(f"User {user_id} not found for code redemption")
                return False
            
            # Mark code as used first to prevent double redemption
            result = await self.premium_codes.update_one(
                {"code": code, "used": False},  # Double check it's still unused
                {"$set": {"used": True, "used_by": user_id, "used_date": datetime.now()}}
            )
            
            if result.modified_count == 0:
                logger.warning(f"Code '{code}' already used by another user")
                return False
            ...
            update_result = await self.update_user(user_id, {"premium_until": new_premium_until})
            
            if update_result:
                logger.info(f"Successfully redeemed code '{code}' for user {user_id}")
                return premium_code["days"]
            else:
                # If user update failed, rollback the code usage
                await self.premium_codes.update_one(
                    {"code": code},
                    {"$set": {"used": False, "used_by": None, "used_date": None}}
                )
                logger.error(f"Failed to update user {user_id} premium status, rolled back code usage")
                return False
                
        except Exception as e:
            logger.error(f"Error in redeem_premium_code: {e}")
            return False
